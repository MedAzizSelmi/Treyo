...
/**
 * Shown after a trainer finishes onboarding, OR when they try to log
 * in while their `approvalStatus` is PENDING. Both paths land here so
 * the messaging stays consistent.
 *
 * No actions to take besides "Back to sign in" — once the admin
 * approves, the trainer's next login will pass the gate and route
 * them to the trainer tabs as usual.
 */
        ...
        <View style={styles.container}>
            ...
            <View style={styles.content}>
                <View style={styles.iconCircle}>
                    <Ionicons name="time-outline" size={48} color="#7cce06" />
                </View>
                <Text style={styles.title}>{t('trainerApproval.pendingTitle')}</Text>
                <Text style={styles.body}>{t('trainerApproval.pendingBody')}</Text>
                <Text style={styles.note}>{t('trainerApproval.pendingNote')}</Text>
            </View>
